...
public class Main {
	...
	// public: Access Modifier (erişim belirleyici) // public: Bütün package'lar
	// kullanabilir
	// protected: Sadece aynı package içi kullanım sağlar
	// private: Sadece sınıf içi kullanım sağlar.

	// static: (zorunlu değil) static metodların hepsi program çalışır çalışmaz
	// memoryde adreslenir ve değişmez
	// static olmasa idi, adres nesne oluşunca oluşacaktı

	// final: (zorunlu değil): metodun başında final yazılırsa o metod override
	// edilemez demektir
	// final keyword'ü sınıfın başına gelirse o sınıftan yeni sınıf türetilemez
	// demektir
	// final'ı daha çok değişkenlerde kullanırız mesela
	final static double PI = 3.14;
	final static int DUNYA_YARICAP = 40000;

	// Return type: Zorunludur. Hiçbir şey return etmeyecekse (döndürmeyecekse) void
	// yazılmalıdır.

	// hello: Metod ismi yazılmak zorundadır.

	// Parantezler: (hello'dan sonraki parantezler) parantezler içine isterseniz
	// parametre(ler) yazabilirsiniz (opsiyonel)
	// Aşağıdaki hello() örneğinde parametre yok.

	// { } : metodun içeriğini (code body) süslü parantezler içerisine yazmamız
	// gerekir.
	...
	// belirleyen metodu yazalım
	// Method OVERLOADING: metod aşırı yüklenmesi

	public static int randomNumber(int ustLimit) { // java'da aynı metod ismi kullanabiliriz ama parametreleri farklı
													// olmalıdır örnekteki gibi
		Random random = new Random();
...
//	}	
	...
	// Not: Aşağıdaki metod overloading değildir ! ismi farklı olduğu için farklı
	// bir metottur.
	...
	public static void main(String[] args) {
		...
		com.alikaya.HelperClass.hello(); // farklı paketten çağırırken bunu kullanmam lazım
		...
		// eğer YardimciSinif içindeki hello() metodu private veya protected olsa idi
		// ona ulaşamazdık ve çağıramazdık

		// com.alikaya.YardimciSinif.hello2(); bu metodu çağıramam çünkü protected
		// tanımlandı başka package'da

		// com.alikaya.YardimciSinif.hello3(); not visible hatası veriyor private metod
		// olduğu için

		int matSonuc = Main.kareAl(5);
		System.out.println("matSonuc: " + matSonuc);

		System.out.println("Random bir sayı 1 : " + Main.randomNumber()); // bu classtaki metodu bu satır ve aşağıdaki
																			// gibi çağırabilirim
		...
		// DUNYA_YARICAP = 200; --> final olan bir değişken değiştirilemez. her zaman
		// ilk değeri kullanılır.
		...
		// System.out.println(x2); // cannot be resolved to a variable hatası veriyor.
		// bu değişkeni bulamıyor.
		// bir önceki parantez içinde tanımlandığı için ulaşamıyor

		// Soru
		// Kullanıcının girdiği alt ve üst limitler arasında random bir sayı üretip
		// return eden bir metod

		System.out.println(randomNumberV3());

		// Soru
		// Main içinde kullanıcıdan bir String alalım ve bu Stringin tekrarlanma
		// sayısını (tamsayı) alın.
		// Aldığımız bu iki değeri tekrarla isimli bir metoda gönderelim, metod
		// gönderdiğimiz metni tekrar sayısı kadar ekrana yazsın
