...
public class DataTypesAreInteger {
...
//			//Değişken tanımlama
		...
		// 255 0 max min alabileceği değerlerdi ama negatif tamsayılar da tutuluyor. bu
		// geçerli değil.
		// ^: Sign bit. En soldaki bit sign bit (artı mı eksi mi) olarak görev yapar.
		// Sign bit 0 ise pozitif sayıdır. Örn: 00000001 : 1, 00000011 : 3, 01111111 :
		// 127
		// Sign bit 1 ise negatif sayıdır.
		// 10000000 : -128 10000001: -128+1=-127 100000010: -126, 11111111: -1
		// Byte'ın alabileceği değer aralığı: -128 ile 127 arasındadır. -2^7 ile 2^7-1
		// arasındadır
		byteValue = 127;
		...
		// Büyük ilkel veritipi = küçük ilkel veritipi diyebiliriz, ancak tersini
		// yapamayız
		shortValue = byteValue; // ok bunu yaabiliyor

		// byteValue = shortValue; // Direkt olarak bu şekilde atama yapamayız.
		// sağdakinin değeri daha büyük. casting gerekiyor...
		byteValue = (byte) shortValue; // sağdakinin başına veri tipini yazarsam atama yapmama izin veriyor.
										// çevireceğim short değerinin
										// byte'ın alabileceği değer aralığında olduğundan eminsem yapmalıyım, java hata
										// vermiyor
										// bana bırakıyor.
		System.out.println(byteValue);
		...
		shortValue = 129; // 0000 0000 1000 0001 = 129 ama byte için 1000 0001 = -127 anlamı ifade eder.
							// Yanlış işlem yapmış oluruz. Dolayısıyla byte'ın alabileceği max değerler
							// dışında kullanma.
		...
		// shortValue = intValue;
		// shortValue = (short) intValue; // intValue, shortValue'nun alabileceği
		// değerler aralığında olmalı

		// Byte toplama
		byteValue = 12;
		byte b2;
		// b2 = byteValue + byteValue; yapılamaz hata verir. elde edilecek toplam byte
		// değer aralığında olsa bile. Neden?
		// çünkü java sağ tarafta byte + byte türünde toplama gördüğünde bunun sonucunu
		// otomatik olarak int'e atar
		b2 = (byte) (byteValue + byteValue); // iki byte'ı toplayacaksam cating ile yazdırabilirim ama toplamın byte'ın
												// alabileceği değer
		...
		short sh2 = 2;
		short sh3;
		// sh3 = sh1 + sh2; // iki short'u toplamaya çalışırken hata alırız, casting
		// yapmamız gerekir
		// casting işlemi yaparken java insiyatifi bize bırakır
		// kontrolü bizim sağlamamız gerekir. toplam değer short'un alabileceği
		// değerleri geçmemeli.
		...
		// longValue = 9223372036854775807; // bu aslında long'un alabileceği vir değer
		// ama integer'ın alabileceği değerlerden yüksek
		// değerleri atayamıyorum. atamaları da java değerine göre yapıyor
		// int'in sınırlarını geçtiğinde hata alıyorum
		longValue = 9223372036854775807L; // sonuna büyük L ya da küçük l harfi yazarak bu long sayısının atamasını
											// yapabilirim

		// Şu tarz hatalar da olabilir:
		// long myTimer = 372036854775807 * 2; => hatalı çünkü 3 ile başlayan sayı
		// int'in alabileceği değer aralığı dışında
